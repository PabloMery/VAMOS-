...
export async function createGroup(eventoId: string): Promise<Grupo> {
  return apiRequest<Grupo>('/usuarios/grupos/', {
    method: 'POST',
...
export async function joinGroupByCode(inviteCode: string): Promise<Grupo> {
  return apiRequest<Grupo>('/usuarios/grupos/unirse/', {
    method: 'POST',
...
export async function getGroupDetail(groupId: string): Promise<GrupoConMiembros> {
  return apiRequest<GrupoConMiembros>(`/usuarios/grupos/${groupId}/`);
}

/** Todos los grupos en los que está el usuario actual. */
export async function getMyGroups(): Promise<Grupo[]> {
  return apiRequest<Grupo[]>('/usuarios/grupos/mis-grupos/');
}
...
export async function updateMyStatus(
  ...
  return apiRequest<MiembroGrupo>(`/usuarios/grupos/${groupId}/estado/`, {
    method: 'PATCH',
    body: { estado },
  });
}
...
export async function leaveGroup(groupId: string): Promise<void> {
  await apiRequest<void>(`/usuarios/grupos/${groupId}/salir/`, {
    method: 'DELETE',
  });
}