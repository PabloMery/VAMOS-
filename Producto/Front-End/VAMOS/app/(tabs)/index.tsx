...
const { saveEvent, confirmEvent, isSaved, isConfirmed } = useSavedEvents();
  ...
  >
    {events.map((event) => {
      // Calculamos el estado antes de retornar el componente
      const status: EventStatus = isConfirmed(event.id_externo)
        ? "confirmado"
        : isSaved(event.id_externo)
        ? "guardado"
        : "neutral";
      ...
      return (
        <EventMarker
          ...
          event={event}
          status={status} // Pasamos el nuevo prop
          onPress={setSelectedEvent} // Mantenemos tu función original
        />
      );
    })}
  </MapView>
